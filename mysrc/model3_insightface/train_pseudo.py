    ...
    while True:
        ...
        for start in range(0, train_size, batch_size):
            ...
            for i in range(start,end,1):
                x_batch = np.vstack((x_batch, xtrain_fold[i, randint(0, 99), :].reshape(1,512)))
...
if __name__ == '__main__':
    ...
    xtrain_pseudo = np.load('pseudo_train_data.npy')
    ...
    training_log = open('training_pseudo_log.txt', 'w')
    loss_average = 0.0
    acc_average = 0.0
    for part in random.sample(range(30), NUMBER_OF_PARTS):
        for fold in range(NUMBER_OF_FOLDS):
            ...
            xtrain_fold = np.vstack((xtrain_fold,xtrain_pseudo))
            ytrain_fold = np.vstack((ytrain_fold,ytrain_pseudo))
            xtrain_fold, ytrain_fold = shuffle(xtrain_fold, ytrain_fold)
    ...
    np.save('ptest_pseudo.npy', ptest)
