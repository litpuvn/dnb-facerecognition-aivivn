...
def my_process1(file_name):
    emb_path = '../../models/vgg_face2/resnet50_128_pytorch/embedding/train/%s'%file_name.replace('.png', '.npy')
    emb = np.load(emb_path)
    return emb

def my_process2(file_name):
    emb_path = '../../models/vgg_face2/resnet50_128_pytorch/embedding/train/%s'%file_name.replace('.png', '_flip.npy')
    emb = np.load(emb_path)
    return emb

def my_process3(file_name):
    emb_path = '../../models/vgg_face2/resnet50_128_pytorch/embedding/test/%s'%file_name.replace('.png', '.npy')
    emb = np.load(emb_path)
    return emb

def my_process4(file_name):
    emb_path = '../../models/vgg_face2/resnet50_128_pytorch/embedding/test/%s'%file_name.replace('.png', '_flip.npy')
    emb = np.load(emb_path)
    return emb
