...
def my_process1(file_name):
    emb_path = '../../models/insightface/embedding/model-r34-amf/test/%s'%file_name.replace('.png', '.npy')
    emb = np.load(emb_path).reshape(512)
    return emb

def my_process2(file_name):
    emb_path = '../../models/insightface/embedding/model-r34-amf/test/%s'%file_name.replace('.png', '_flip.npy')
    emb = np.load(emb_path).reshape(512)
    return emb

def my_process3(file_name):
    emb_path = '../../models/insightface/embedding/model-r34-amf/train/%s'%file_name.replace('.png', '.npy')
    emb = np.load(emb_path).reshape(512)
    return emb

def my_process4(file_name):
    emb_path = '../../models/insightface/embedding/model-r34-amf/train/%s'%file_name.replace('.png', '_augmentation.npy')
